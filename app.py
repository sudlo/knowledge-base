...
KB = {
    ...
    "Cloud": {
        "AWS": {
            "color": AMBER,
            "Compute":    ["EC2", "Lambda", "ECS", "EKS", "Fargate", "Batch", "Elastic Beanstalk", "Lightsail", "Outposts", "AppRunner", "Serverless App Repo", "VMware Cloud on AWS", "Wavelength", "Local Zones"],
            "Storage":    ["S3", "EBS", "EFS", "FSx", "S3 Glacier", "Storage Gateway", "AWS Backup", "Snow Family", "Elastic Disaster Recovery", "DataSync"],
            "Networking": ["VPC", "Route 53", "CloudFront", "ALB/NLB/ELB", "Direct Connect", "Transit Gateway", "PrivateLink", "Global Accelerator", "API Gateway", "App Mesh", "Cloud Map", "Client VPN", "Site-to-Site VPN"],
            ...
            "Security":   ["IAM", "KMS", "Secrets Manager", "GuardDuty", "Security Hub", "WAF", "Shield", "Macie", "Inspector", "Cognito", "Certificate Manager", "Directory Service", "Network Firewall", "Detective"],
            "Monitoring": ["CloudWatch", "X-Ray", "CloudTrail", "AWS Config", "Trusted Advisor", "Cost Explorer", "AWS Health", "Control Tower", "Compute Optimizer", "Managed Grafana", "Managed Prometheus"],
            "AI / ML":    ["SageMaker", "Bedrock", "Rekognition", "Polly", "Comprehend", "Translate", "Lex", "Kendra", "Forecast", "Personalize", "Textract", "Transcribe"]
        },
        "Azure": {
            "color": BLUE,
            "Compute":    ["Virtual Machines", "Azure Functions", "AKS (Kubernetes)", "Container Apps", "App Service", "Azure Batch", "Service Fabric", "Spring Apps", "VM Scale Sets", "Azure Dedicated Host", "Azure Spot VMs"],
            "Storage":    ["Blob Storage", "Azure Disk", "Azure Files", "NetApp Files", "Data Lake Storage Gen2", "Archive Storage", "Azure Backup", "Import/Export", "Data Box", "StorSimple"],
            "Networking": ["Virtual Network (VNet)", "Azure DNS", "Azure CDN", "Load Balancer", "Application Gateway", "ExpressRoute", "Azure Firewall", "DDoS Protection", "Traffic Manager", "Virtual WAN", "VPN Gateway", "Bastion", "Private Link"],
            ...
            "DevOps":     ["Azure Pipelines", "GitHub Actions", "Azure Repos", "Azure Artifacts", "Bicep / ARM Templates", "Container Registry (ACR)", "DevTest Labs", "Load Testing", "Azure Boards", "Dev Box"],
            "Security":   ["Entra ID (Active Directory)", "Key Vault", "Defender for Cloud", "Microsoft Sentinel", "Azure Policy", "Privileged Identity Management (PIM)", "Confidential Computing", "Information Protection"],
            "Monitoring": ["Azure Monitor", "Application Insights", "Log Analytics", "Azure Advisor", "Cost Management", "Service Health", "Network Watcher", "Resource Graph"],
            "AI / ML":    ["Azure OpenAI Service", "Machine Learning Studio", "Cognitive Search", "Computer Vision", "Speech Services", "Language Understanding (LUIS)", "Bot Service", "Form Recognizer"]
        },
        "Google Cloud": {
            "color": GREEN,
            "Compute":    ["Compute Engine", "Cloud Run", "GKE (Kubernetes Engine)", "Cloud Functions", "App Engine", "Cloud Batch", "Bare Metal", "VMware Engine", "Preemptible VMs", "Spot VMs"],
            ...
            "Networking": ["Virtual Private Cloud (VPC)", "Cloud DNS", "Cloud CDN", "Cloud Load Balancing", "Cloud Interconnect", "VPC Service Controls", "Cloud Armor", "Network Intelligence Center", "Cloud NAT", "Cloud VPN", "Service Directory"],
            ...
            "Security":   ["IAM", "Cloud KMS", "Secret Manager", "Security Command Center", "Binary Authorization", "Assured Workloads", "Chronicle SIEM", "VPC Service Controls", "Identity-Aware Proxy (IAP)"],
            ...
            "AI / ML":    ["Vertex AI", "Gemini API", "AutoML", "Vision AI", "Speech-to-Text", "Text-to-Speech", "Translation AI", "Natural Language AI", "Dialogflow", "Recommendations AI", "Document AI"]
        },
    },
    "DevOps": {
        "CI/CD": {
        ...
        if section == "Cloud":
            ...
            for os_name, osdata in section_data.items():
                for topic, tdata in osdata["topics"].items():
                    ...
                    index.append({
                        "title": topic, "section": section, "parent": os_name,
                        "text": blob, "type": "topic", "data": tdata, "color": osdata["color"],
                    })
        elif section == "DevOps":
            for cat, tools in section_data.items():
                for tool_name, tdata in tools.items():
                    ...
                    index.append({
                        "title": tool_name, "section": section, "parent": cat,
                        "text": blob, "type": "devops_tool", "data": tdata, "color": tdata["color"],
                    })
        elif section in ("Networking", "Database"):
            for topic, tdata in section_data.items():
                blob = " ".join([topic, tdata["desc"]]).lower()
                index.append({
                    "title": topic, "section": section, "parent": section,
                    "text": blob, "type": "topic", "data": tdata, "color": tdata["color"],
                })
    return index
...
class KnowledgeBase(tk.Tk):
    ...
    def _row_btn(self, parent, text, sub="", color=TEXT2, cmd=None):
        ...
        tk.Label(row, text=text, font=F_BODY, fg=TEXT1, bg=CARD, padx=14, pady=10, anchor="w").pack(side=tk.LEFT, fill=tk.X, expand=True)
        if sub:
    ...
    def _detail_win(self, title, parent_label, data, is_devops=False):
        ...
        tk.Label(hdr, text=data.get("desc", ""), font=F_BODY, fg=TEXT2, bg=CARD, wraplength=820, justify=tk.LEFT).pack(anchor="w", pady=(4, 0))
        ...
        tk.Button(btn_row, text="Web search", font=F_SMALL, fg=ACCENT2, bg=TAG, bd=0, padx=10, pady=6, cursor="hand2", command=lambda: self._open_web(title)).pack(side=tk.LEFT, padx=(0, 6))
        tk.Button(btn_row, text="Ask IronHide", font=F_SMALL, fg=TEXT1, bg=ACCENT, bd=0, padx=10, pady=6, cursor="hand2", command=lambda: [win.destroy(), self._show("agent"), self._prefill(f"Deep dive on {title}")]).pack(side=tk.LEFT)
    ...
    def _load_cloud(self, provider):
        ...
        for cat, services in data.items():
            ...
            for i, svc in enumerate(services):
                r, c = divmod(i, 5) 
                card = tk.Frame(grid, bg=CARD, padx=10, pady=8, cursor="hand2")
    ...
    def _build_logs(self):
        ...
        input_container = tk.Frame(f, bg=CARD, padx=10, pady=10)
        input_container.pack(fill=tk.X, pady=(0, 10))
        
        tk.Label(input_container, text="Paste raw logs, JSON dumps, or ticket details here:", font=F_SMALL, fg=TEXT2, bg=CARD).pack(anchor="w", pady=(0, 5))
        
        self._log_input = scrolledtext.ScrolledText(
            input_container, font=F_MONO, bg=INPUT, fg=TEXT1, bd=0,
            padx=10, pady=10, wrap=tk.WORD, height=8, insertbackground=TEXT1
        )
        ...
        tk.Button(btn_row, text="Upload File (.log/.txt)", font=F_SMALL, fg=TEXT1, bg=PANEL, bd=0, padx=14, pady=8, cursor="hand2", command=self._load_log_file).pack(side=tk.LEFT, padx=(0, 10))
        tk.Button(btn_row, text="Clear", font=F_SMALL, fg=TEXT3, bg=CARD, bd=0, padx=14, pady=8, cursor="hand2", command=lambda: self._log_input.delete("1.0", tk.END)).pack(side=tk.LEFT)
        
        self._analyze_btn = tk.Button(btn_row, text="Analyze with IronHide", font=F_BODY, fg="white", bg=ACCENT, bd=0, padx=20, pady=8, cursor="hand2", command=self._run_log_analysis)
        self._analyze_btn.pack(side=tk.RIGHT)

        tk.Label(f, text="*Note: Image/Diagram upload is not supported in the free tier. Please describe diagrams in text.*", font=("Segoe UI", 9, "italic"), fg=TEXT3, bg=BG).pack(anchor="w", pady=(0,5))

        output_container = tk.Frame(f, bg=CARD, padx=10, pady=10)
        output_container.pack(fill=tk.BOTH, expand=True)
        
        tk.Label(output_container, text="Analysis & Resolution Steps:", font=F_SMALL, fg=TEXT2, bg=CARD).pack(anchor="w", pady=(0, 5))
        
        self._log_output = scrolledtext.ScrolledText(
            output_container, font=F_MONO, bg=PANEL, fg=GREEN, bd=0,
            padx=14, pady=10, wrap=tk.WORD, state=tk.DISABLED, insertbackground=TEXT1
        )
    ...
    def _run_agent(self):
        ...
        try:
            ...
            messages = [{
                "role": "system", 
                "content": "You are IronHide, a Senior DevOps Engineer and coding assistant. Write clean, production-ready scripts. You are an expert in Python, Bash, PowerShell, Terraform, Ansible, and Go. Keep explanations extremely brief. Provide exactly the code requested in the language requested."
            }]
